...
def find_latest_log():
    logs_dir = os.path.join(os.path.dirname(MODULAR_DIR), "logs")
    logs = sorted(
        glob.glob(os.path.join(MODULAR_DIR, "flight_log_*.json"))
        + glob.glob(os.path.join(MODULAR_DIR, "flight_log_*.md"))
        + glob.glob(os.path.join(logs_dir, "flight_log_*.json"))
        + glob.glob(os.path.join(logs_dir, "flight_log_*.md")),
...
def load_log(filepath):
    if filepath.endswith(".json"):
        ...
        table_start = next(
            (i for i, l in enumerate(lines) if l.strip().startswith("| Time")), None
        )
        table = []
...
def write_markdown_report(log_data, alt_img, turb_img, outpath):
    with open(outpath, "w", encoding="utf-8") as f:
        ...
        if "regime_label" in log_data:
            f.write(f"## 🔄 Market Synchronization\n\n")
            for k in [
                "regime_label",
        ...
        f.write(
            "| Time | Altitude (%) | Fuel (%) | Stall | Turbulence | Phase | Status | Regime |\n"
        )
        f.write(
            "|------|--------------|----------|-------|------------|-------|--------|--------|\n"
        )
        for i, row in enumerate(log_data["telemetry"]):
            regime = ""
            if "synchronization" in log_data and i < len(log_data["synchronization"]):
                regime = log_data["synchronization"][i].get("regime_label", "")
            f.write(
                f"| {row['time']} | {row['altitude']:+.2f} | {row['fuel']:.1f} | {'Yes' if row['stall'] else 'No'} | {row['turbulence']} | {row['phase']} | {row['status']} | {regime} |\n"
            )
        f.write("\n")
...
if __name__ == "__main__":
    ...
    if latest_log.endswith(".json"):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                "| Time | Altitude (%) | Fuel (%) | Stall | Turbulence | Phase | Status |\n"
            )
            f.write(
                "|------|--------------|----------|-------|------------|-------|--------|\n"
            )
